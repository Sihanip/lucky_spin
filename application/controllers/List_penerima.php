...
class List_penerima extends CI_Controller {

	public function __construct()
    {
        parent::__construct();
        $this->load->model('m_list_penerima');
        is_logged_in();
    }

	public function index()
	{
		$data['title'] = "List Penerima hadiah";
		$data['konten'] = "List Penerima hadiah";
		$data['list_toko'] = $this->m_list_penerima->list_toko();
		$data['list_hadiah'] = $this->m_list_penerima->list_hadiah();
		$this->load->view('template/header', $data);
		$this->load->view('list_penerima_view');
    ...
    function add_hadiah()
    {
		...
		$result = $this->m_list_penerima->add_hadiah($data_hadiah);
		if($result > 0)
		{
			redirect('list_penerima');
		}
		else
		{
		}
    ...
    function simpan_customer()
    {
		...
		$result = $this->m_list_penerima->simpan_customer($data_cust);
    ...
    function edittoko()
    {
		...
		$result = $this->m_list_penerima->edittoko($data_toko,$id);
		redirect('list_penerima');
	}
    function get_toko()
    {
		$toko_id	= $this->input->post('toko_id');
		$data1			= $this->m_list_penerima->get_toko($toko_id);
		$data				= array();
			if ($data1->num_rows() > 0) {
    ...
    function get_list_penerima()
    {
		...
		$data1			= $this->m_list_penerima->get_list_penerima($toko_id);
		$data				= array();
			if ($data1->num_rows() > 0) {
				foreach ($data1->result_array() as $row) {
					$data[] = $row;
    ...
    function get_cek_toko()
    {
		...
		$data1			= $this->m_list_penerima->get_cek_toko($id_toko,$id_hadiah);
		$data				= array();
			if ($data1->num_rows() > 0) {
				foreach ($data1->result_array() as $row) {
					$data[] = $row;
